...
import { CertificateResult } from '@/types/quiz';
...
const CertificateCard = styled.div<{ $tier: string }>`
  ...
  &::before {
    ...
    background: ${(props) => {
      ...
      if (tier === 'gold') return props.theme.colors.certificate.gold.gradient;
...
const Subtitle = styled.h2<{ $tier: string }>`
  ...
  color: ${(props) => {
    ...
    if (tier === 'gold') return props.theme.colors.certificate.gold.accent;
  ...
  return (
    <Container>
      <CertificateCard $tier={result.tier}>
        ...
        <Subtitle $tier={result.tier}>AZ Certified – {result.tier} Level</Subtitle>
        ...
        <ScoreText>Score: {result.score}%</ScoreText>
